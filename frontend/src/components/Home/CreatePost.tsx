...
function CreatePost(props: any) {
  ...
  useEffect(() => {
    (async () => {
      if (id) {
        ...
        const response = await fetch(`${GET_POST_ROUTE}${id}`, requestOptions);
  ...
  const handleCreatePost = async () => {
    console.log(
      articalInfo,
      !articalInfo?.title?.trim?.(),
      !articalInfo?.description?.trim?.(),
      !articalInfo?.category?.trim?.(),
      !fixedCategory.includes(articalInfo?.category?.trim())
    );
    if (
      !articalInfo?.title?.trim?.() ||
      !articalInfo?.description?.trim?.() ||
      ...
      const response = await fetch(CREATE_POST_ROUTE, requestOptions);
  ...
  const handleUpdatePost = async () => {
    ...
    if (
      ...
      const response = await fetch(`${UPDATE_POST_ROUTE}${id}`, requestOptions);
      const reposnseData = await response.json();
      if (reposnseData.success) {
        navigate("/");
      } else {
      ...
      >
        ...
        >
          <TextField
            ...
            onChange={(e) => {
              setArticalInfo({ ...articalInfo, title: e.target.value });
            }}
            label="Title"
            variant="outlined"
          />
          <TextField
            multiline
            id="outlined-basic"
            value={articalInfo.description}
            onChange={(e) => {
              setArticalInfo({ ...articalInfo, description: e.target.value });
            }}
            label="Description"
            variant="outlined"
          />
          <FormControl fullWidth>
            ...
            <Select
              ...
              label="Category"
              onChange={(e) => {
                setArticalInfo({ ...articalInfo, category: e.target.value });
              }}
            >
